...
while running:
  ...
  for i in range(game.height):
      for j in range(game.width):
        pygame.draw.rect(screen, (125,125,125), [game.x + game.zoom * j, game.y + game.zoom * i, game.zoom, game.zoom], 1)
        if game.field[i][j] > 0:
  ...
  text_game_esc = font2.render("Press ESC to Restart", True, (255,255,255))

  screen.blit(text, [0,0])
  if game.state == "gameover":
    screen.blit(text_game_over, [80,200])
    screen.blit(text_game_esc, [90,265])
  pygame.display.flip()